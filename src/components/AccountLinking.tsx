...
export function AccountLinking() {
  ...
  // Check if user has only one account type signed in
  ...
  const handleLinkAccounts = async () => {
    ...
    try {
      ...
      // Update the session to reflect the linked accounts
      await update({
        ...session,
        user: {
          ...session.user,
  ...
  // Function to initiate Spotify sign-in
  ...
  // Only has Farcaster - needs Spotify
  if (hasOnlyFarcaster) {
    return (
  ...
  // Only has Spotify - needs Farcaster
  if (hasOnlySpotify) {
    return (
      <div className="p-4 bg-blue-800/20 rounded-lg mb-4">
...
// Simple button to launch Farcaster sign-in
